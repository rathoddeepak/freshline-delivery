  ...
  public void Show(final ReadableMap props, final Callback callback) {
    ...
    if (presentationType < 3) {
      if (presentationType == 0) {
        ...
        alert = new AlertView(title, subtitle, AlertStyle.IOS);
      }
      ...
      for (int i = 0; i < values.size(); i++) {
        ReadableMap map = values.getMap(i);
        String value = map.getString("value");
        AlertActionStyle type = AlertActionStyle.DEFAULT;
        if(map.hasKey("type")){
          if(map.getInt("type") == 1){
            type = AlertActionStyle.POSITIVE;
          }else if(map.getInt("type") == 2){
            type = AlertActionStyle.NEGATIVE;
          }
        }
        alert.addAction(new AlertAction(value, type, new AlertActionListener() {
          @Override
