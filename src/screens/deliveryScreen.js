...
export default class DeliveryScreen extends Component {
	...
	componentDidMount(){
		const {delivery_id} = this.props.route.params;
		this.setState({delivery_id}, this.loadData);
	}
	...
	loadData = () => {
		...
		Parse.Cloud.run("getDeliveryData", {delivery_id}).then(data => {
	...
	setData = (data) => {
		const {
			id,title,task_id,address,agent_status,items,time,pay_mode,delivery_fee,phone_no,tip_amt,total_amt,location,cl_address,landmark,flat
		} = data;		
		let dataList = [];
		...
		dataList.push({title:"Address", data:cl_address});
		if(landmark != undefined && landmark.length != 0)dataList.push({title:"Landmark", data:landmark});
		if(flat != undefined && flat.length != 0)dataList.push({title:"Flat", data:flat});
		dataList.push({title:"Delivery Fee", data:delivery_fee});
		dataList.push({title:"Pay Mode", data:helper.COD == pay_mode ? "COD" : "ONLINE"});
		dataList.push({title:"Amount", data:total_amt});
		if(tip_amt > 0)dataList.push({title:"Tip", data:tip_amt});
	...
	renderHeader = () => {
		...
		<>
		 ...
		 <View style={{height:270,width:'100%',overflow:'hidden'}}>
			  ...
			  {showAction ? <TouchableOpacity onPress={() => this.address(address.lat, address.lng)} style={{backgroundColor:helper.blackTrans,borderRadius:10,position:'absolute',bottom:6,right:6,width:40,height:40,justifyContent:'center',alignItems:'center'}}>
	     ...
	     <View style={{width:'98%',alignSelf:'center',paddingTop:5,flexDirection:'row',paddingLeft:5,paddingBottom:10,marginTop:5,borderBottomWidth:0.5,borderColor:helper.grey6,justifyContent:'space-between'}}>
	        <View style={{width:'70%'}}>
		        ...
		        <Text style={{fontSize:14,fontFamily:'sans-serif-medium',color:helper.silver}}>{time} - Delivery - <Text style={{color}}>{text}</Text></Text>
		    </View>
		    {showAction ? <TouchableOpacity onPress={() => this.call(customer.phone)} style={{height:50,justifyContent:'center',alignItems:'center'}}>
...
class TaskActionBar extends Component {
	      ...
	      [
	        {
	          text: "YES",
	          onPress: () => {
	          	this.cloudPerform('cancelTask', helper.FLEETX_CANCEL, {task_id,hero_id,full_refund:false});
	          }
	        },
	        {
	        	text: "NO",
	        	onPress: () => {
	        		this.cloudPerform('cancelTask', helper.FLEETX_CANCEL, {task_id,hero_id,full_refund:true});
	        	}
	        }	        
	      ]
	    )		
	...
	cloudPerform = async (cloudAction, newStatus, extra = {}) => {
		try {
			this.setState({busy:true})						
			const response = await request.taskAction(cloudAction, {hero_id:heroId,delivery_id:this.props.id, ...extra});
			this.setState({busy:false}, () => {
				this.panX.setValue(0);
				if(response){
